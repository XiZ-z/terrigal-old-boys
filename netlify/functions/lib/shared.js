...
async function readScoreSheet(base64Data, mediaType, isFinals, teamLabels, expectedContext) {
  ...
  const contextNote = expectedContext
    ...
    : '';
  const prompt = `You are reading a photo of a paper score sheet for a club tennis competition. Each side has a SCORE table listing individual pairing rows, ending in that table's own "Total" row -- that Total row has two separate values side by side, one under a "GAMES" column header and one under a "POINTS" column header. Read setsA/setsB from the Total row's POINTS value and gamesA/gamesB from the Total row's GAMES value, for each side. Ignore player names and the individual pairing rows above the Total row.
Below the SCORE table there is a separate "BONUS POINTS" box, then a bigger standalone "TOTAL" box of its own -- that bigger TOTAL box adds the Total row's POINTS value plus Bonus Points together (the match's grand total), so it is a DIFFERENT number to the sets figure, never a substitute for it. Also read that bigger standalone TOTAL box's value for each side and report it as sheetTotalA/sheetTotalB (a number, or null if illegible/blank) -- purely so it can be cross-checked against the Total row afterward, never use it as setsA/setsB or gamesA/gamesB. Ignore the Bonus Points box itself.${teamMatchNote}${contextNote}
...
Respond with ONLY a single JSON object, no other text, no markdown code fences, in exactly this shape: {"setsA": number, "setsB": number, "gamesA": number, "gamesB": number, "sheetTotalA": number or null, "sheetTotalB": number or null${isFinals ? ', "winner": "A" or "B"' : ''}${expectedContext ? ', "courtOnSheet": number or null, "dateMatches": true, false, or null' : ''}}`;
